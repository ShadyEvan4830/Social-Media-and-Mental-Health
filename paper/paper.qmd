...
Observing the trend in the graph @fig-too-little-trend, we note an undulating pattern of "too little" responses over the fifty-year span. The trend is characterized by significant peaks and troughs, it is notable that there are periods where the counts sharply increase, suggesting spikes in the sentiment that welfare is insufficient. Following these peaks, there are also steep declines at certain intervals, indicating a drop in the number of "too little" responses. Overall, despite these fluctuations, there is a discernible upward trend in the recent 10 years (2012-2022), culminating in a prominent peak towards the end of the period under consideration. This trend suggests an increasing number of respondents perceive welfare provisions to be inadequate in more recent years.
...
Our data analysis found trends in the “too little” category of General Social Survey responses that could indicate public perceptions of benefit adequacy. This trend corresponds to the changing economic landscape over the years, and to capture this dynamic quantitatively, we designed a linear regression model to predict the likelihood of a “too few” response depending on the year of survey. We ran the model in R [@citeR] using @rstanarm's `rstanarm` package. For comprehensive background details on model construction, variables, and diagnostics, please see the [Appendix](#sec-model-details).
...
The linear regression model we propose serves to validate this hypothesis mathematically. If the coefficient $\beta_1$ to be positive will confirm our assumption of an upward trajectory for “too few” responses. As an illustrative example, suppose that the GSS data show a large increase in “too few” responses in the years following the implementation of the welfare reform policy, which would appear in our model as a steeper slope in these years.
...
#| tbl-cap: The model trend of Too Little responses is increasing every years
...
#| fig-cap: The trend of Too Little responses is increasing every years
